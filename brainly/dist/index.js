...
const generateHash = async () => {
    ...
    let isUnique = false;
    while (!isUnique) {
        hash = crypto_1.default.randomBytes(4).toString('hex');
        const existingLink = await db_1.linkModel.findOne({ hash });
        if (!existingLink) {
            isUnique = true;
        }
    }
...
app.post("/api/v1/signup", async (req, res) => {
    try {
        const validateData = zodValidation_1.userRegValidator.parse(req.body);
        const existingUser = await db_1.userModel.findOne({ username: validateData.username });
        if (existingUser) {
            return res.status(403).json({ message: "User already exists with this username, try sign in" });
        }
        const hashedPassword = await bcrypt_1.default.hash(validateData.password, 10);
...
app.post("/api/v1/signin", async (req, res) => {
    try {
        const validateData = zodValidation_1.userLoginValidator.parse(req.body);
        const existingUser = await db_1.userModel.findOne({
            username: validateData.username,
        });
        if (existingUser) {
            const isPasswordValid = await bcrypt_1.default.compare(validateData.password, existingUser.password);
            if (isPasswordValid) {
...
app.post("/api/v1/content", userMiddleware_1.userMiddleware, async (req, res) => {
    try {
        const validateData = zodValidation_1.contentValidator.parse(req.body);
        const validateTag = zodValidation_1.tagValidator.parse(req.body);
...
app.get("/api/v1/content", userMiddleware_1.userMiddleware, async (req, res) => {
    const contentId = req.query.contentId;
    if (!contentId) {
        return res.status(400).json({ message: "Valid Content ID is required" });
    }
    try {
        const existingContent = await db_1.contentModel.findOne({
...
app.put("/api/v1/content", userMiddleware_1.userMiddleware, async (req, res) => {
    const contentId = req.query.contentId;
    if (!contentId) {
        return res.status(400).json({ message: "Valid Content ID is required" });
    }
...
app.delete("/api/v1/content", userMiddleware_1.userMiddleware, async (req, res) => {
    const contentId = req.query.contentId;
    if (!contentId) {
        return res.status(400).json({ message: "Valid Content ID is required" });
    }
    try {
        const existingContent = await db_1.contentModel.findOneAndDelete({
            _id: contentId, // Use _id, not contentId
            userId: req.userId // req.userId is already a string
        });
        if (!existingContent) {
        ...
        await db_1.linkModel.deleteMany({ contentId: contentId });
...
// SHARE ROUTES
app.post("/api/v1/brain/share", userMiddleware_1.userMiddleware, async (req, res) => {
    try {
...
app.get("/api/v1/brain/:shareLink", async (req, res) => {
    try {
        ...
        const link = await db_1.linkModel.findOne({ hash: shareLink })
...
app.get("/api/v1/brain/share/all", userMiddleware_1.userMiddleware, async (req, res) => {
    try {
        const shareLinks = await db_1.linkModel.find({
            userId: req.userId // req.userId is already a string
